	...
	switch (id) {
        ...
        case "ipt_repwd":
            if ($.trim(val) == "") {
                $("#reg_noti_msg").css('visibility','hidden');
                ItemStat.repwd = 0;  
            } else if (val!=$("#ipt_pwd").val()) {
                ItemStat.repwd = -1;
				$("#"+id).addClass('err-ipt');
                showMsg("两次输入的密码不一致","err_msg");
            } else {
                $("#reg_noti_msg").css('visibility','hidden');
                ItemStat.repwd = 1;
            }
...
function reCkeckForm(){
	...
	else //如果表单验证成功
	{
		...
		if(portraitVal=="-1"){   //如果为默认值，代表没有上传头像
			showMsg("请上传您的头像图片","err_msg");
		}else{ //头像检测成功
			...
			checkExist().then(function(res){
				...
				if(currentStep==3){
					...
					if(res.key==""){
						...
						var regUrl=APIurl+"user/regist";
						var regformStr=$("#reg_form").serialize()+"&userName="+$("#nickname").val()+"&portrait="+upLoadUrl;
...
function disableBtn(isTrue){
	if(isTrue){
		$("#btn_doreg").val("注册中...").attr('disabled',true);
	}else
	{
		$("#btn_doreg").val("注册").attr('disabled',false);	
	}
}
